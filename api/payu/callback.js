...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!salt || !merchantKey) {
      ...
      return res.redirect(302, `${baseUrl}/?status=error&message=ServerConfigError`);
    }

    // Verify reverse hash:
    // sha512(salt|status||||||udf5|udf4|udf3|udf2|udf1|email|firstname|productinfo|amount|txnid|key)
    let hashString = `${salt}|${status}|${udf5}|${udf4}|${udf3}|${udf2}|${udf1}|${email}|${firstname}|${productinfo}|${amount}|${txnid}|${merchantKey}`;
    ...
    if (calculatedHash !== hash) {
      console.error('PayU Hash mismatch. Expected:', calculatedHash, 'Received:', hash);
      return res.redirect(302, `${baseUrl}/?status=tampered`);
    }
    ...
    if (status === 'success') {
      ...
      if (order && order.status === 'paid' && order.email_sent === true) {
        return res.redirect(302, `${baseUrl}/?order_id=${encodeURIComponent(txnid)}&check_status=true&amount=${orderAmount}`);
      }
